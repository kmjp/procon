...
void solve() {
	...
	Y=K-r;
	Z=K-x;
	if(X>R||Y>G||Z>B||X+Y+Z>K) return _P("0\n");
	
	vector<ll> Gs,Bs;
	FOR(i,G+1) {
		if(i<Y) Gs.push_back(0);
		else Gs.push_back(comb(G,i));
	}
	FOR(i,B+1) {
		if(i<Z) Bs.push_back(0);
		else Bs.push_back(comb(B,i));
	}
	auto GB=MultPoly(Gs,Bs,1);
	GB.resize(1<<20);
	ll ret=0;
	for(i=X;i<=R;i++) if(K-i>=0) {
		(ret+=comb(R,i)*GB[K-i])%=mo;
	}
	cout<<ret<<endl;
