...
vector<ll> fftv(vector<ll> v, bool rev=false) {
	int n=v.size(),i,j,m;
...
vector<ll> MultPoly_and(vector<ll> P,vector<ll> Q,bool resize=false) {
	P=fft_and(P), Q=fft_and(Q);
	for(int i=0;i<P.size();i++) P[i]*=Q[i];
	return fft_and(P,true);
}

vector<ll> fft_xor(vector<ll> v, bool rev=false) {
	int n=v.size(),i,j,m;
...
vector<ll> MultPoly_xor(vector<ll> P,vector<ll> Q,bool resize=false) {
	P=fft2_xor(P), Q=fft2_xor(Q);
	for(int i=0;i<P.size();i++) P[i]*=Q[i];
	return fft2_xor(P,true);
}
