...
public:
	...
	V mincost(int from, int to, ll flow) {
		...
		while(flow>0) {
			fill(dist, dist+NV, numeric_limits<V>::max()/5);
			dist[from]=0;
			...
			while(Q.size()) {
				...
				if(d==numeric_limits<V>::max()/5) break;
				FOR(i,E[cur].size()) {
					edge &e=E[cur][i];
			...
			if(dist[to]==numeric_limits<V>::max()/5) return -1;
			V lc=flow;
			for(v=to;v!=from;v=prev_v[v]) lc = min(lc, E[prev_v[v]][prev_e[v]].capacity);
			FOR(i,NV) pot[i]=min(numeric_limits<V>::max()/5,pot[i]+dist[i]);
			flow -= lc;
			res += lc*pot[to];
...
public:
    long long minimumTotalDistance(vector<int>& robot, vector<vector<int>>& factory) {
		...
		FOR(x,M) mcf.add_edge(100+x,201,factory[x][1],0);
		FOR(x,N) FOR(y,M) mcf.add_edge(x,100+y,1,abs(robot[x]-factory[y][0]));
		return mcf.mincost(200,201,N);
