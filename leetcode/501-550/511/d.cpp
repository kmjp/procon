...
pair<ll,ll> rotate_min(string S) {
	// rotateして一番ハッシュ値
	
	string A=S+S;
	rh.init(A);
	pair<ll,ll> p={1LL<<60,1LL<<60};
	int i;
	FOR(i,S.size()) {
		p=min(p,rh.hash(i,i+S.size()-1));
	}
	return p;
}
...
public:
    int minimumGroups(vector<string>& words) {
        set<pair<pair<ll,ll>,pair<ll,ll>>> V;
        FORR(w,words) {
			string A[2];
			int i;
			FOR(i,w.size()) A[i%2].push_back(w[i]);
			
			V.insert({rotate_min(A[0]),rotate_min(A[1])});
		}
		return V.size();
    }
};
