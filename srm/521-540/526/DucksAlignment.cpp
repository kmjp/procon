...
class DucksAlignment {
	...
	int minimumTime(vector <string> grid) {
		int i,y,x,mi=1000000;
		int H=grid.size();
		int W=grid[0].size();
		
		// hor
		vector<pair<int,int> > V;
		FOR(x,W) FOR(y,H) if(grid[y][x]=='o') V.push_back(make_pair(x,y));
		int N=V.size();
		FOR(x,W-(N-1)) FOR(y,H) {
			int r=0;
			FOR(i,N) r+=abs(V[i].first-(x+i))+abs(V[i].second-y);
			mi=min(mi,r);
		}
		V.clear();
		FOR(y,H) FOR(x,W) if(grid[y][x]=='o') V.push_back(make_pair(x,y));
		FOR(x,W) FOR(y,H-(N-1)) {
			int r=0;
			FOR(i,N) r+=abs(V[i].first-x)+abs(V[i].second-(y+i));
			mi=min(mi,r);
		}
		return mi;
	}
