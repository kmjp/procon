...
class StonesOnATree {
	...
	int minStones(vector <int> p, vector <int> w) {
		...
		for(int i=N-1;i>=0;i--) {
			dp[i]=w[i];
			if(E[i].size()==1) dp[i]=max(w[i]+w[E[i][0]],dp[E[i][0]]);
			if(E[i].size()==2) dp[i]=max(w[i]+w[E[i][0]]+w[E[i][1]],min(max(dp[E[i][0]],w[E[i][0]]+dp[E[i][1]]),max(dp[E[i][0]]+w[E[i][1]],dp[E[i][1]])));
		}
		return dp[0];
...
// BEGIN CUT HERE
	...
	void test_case_1() { int Arr0[] = {0,0}; vector <int> Arg0(Arr0, Arr0 + (sizeof(Arr0) / sizeof(Arr0[0]))); int Arr1[] = {1,2,3}; vector <int> Arg1(Arr1, Arr1 + (sizeof(Arr1) / sizeof(Arr1[0]))); int Arg2 = 6; verify_case(1, Arg2, minStones(Arg0, Arg1)); }
	...
	void test_case_3() { int Arr0[] = {0,0,1,1,2,2}; vector <int> Arg0(Arr0, Arr0 + (sizeof(Arr0) / sizeof(Arr0[0]))); int Arr1[] = {1,1,1,1,1,1,1}; vector <int> Arg1(Arr1, Arr1 + (sizeof(Arr1) / sizeof(Arr1[0]))); int Arg2 = 4; verify_case(3, Arg2, minStones(Arg0, Arg1)); }
