...
void up(string& a,string b) {
	if(a=="") a=b;
	else {
		if(a.size()>b.size()) a=b;
		else if(a.size()==b.size() && b<a) a=b;
	}
}
...
void solve() {
	...
	FOR(i,256) {
		cout<<i<<endl;
		FOR(x,256) {
			if(C[x][2].size()) up(C[x][1],C[x][2]);
			if(C[x][1].size()) up(C[x][0],C[x][1]);
			if(C[x][0].size()) up(C[x][2],"("+C[x][0]+")");
			if(C[x][2].size()) up(C[x][1],C[x][2]);
			if(C[x][1].size()) up(C[x][0],C[x][1]);
		}
		FOR(x,256) {
			if(C[x][2].size()) up(C[x^255][2],"!"+C[x][2]);
			
			FOR(y,256) {
				int z=x|y;
				if(C[x][0].size() && C[y][1].size()) up(C[z][0],C[x][0] +"|"+ C[y][1]);
				if(C[y][0].size() && C[x][1].size()) up(C[z][0],C[y][0] +"|"+ C[x][1]);
				z = x&y;
				if(C[x][1].size() && C[y][2].size()) up(C[z][1],C[x][1] +"&"+ C[y][2]);
				if(C[y][1].size() && C[x][2].size()) up(C[z][1],C[y][1] +"&"+ C[x][2]);
