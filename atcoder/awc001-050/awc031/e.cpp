...
void solve() {
	...
	while(Q--) {
		...
		MaxFlow_dinic<ll> mf;
		FOR(i,K) if(ok[i]) mf.add_edge(N+i,N+K+1,W[i]);
		FOR(i,N) mf.add_edge(N+K,i,B[i]);
		FOR(i,M) {
			mf.add_edge(U[i],V[i],C[i]);
			mf.add_edge(V[i],U[i],C[i]);
		}
		
		if(mf.maxflow(N+K,N+K+1)==sum) {
			cout<<"Yes"<<endl;
		}
		else {
			cout<<"No"<<endl;
