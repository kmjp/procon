...
void solve() {
	...
	mcf.add_edge(0,1,M,0);
	mcf.add_edge(0,2,M,0);
	mcf.add_edge(0,3,M,0);
	
	FOR(i,N) {
		mcf.add_edge(1,4+i,1,R[0]-A[i]*B[i]%R[0]);
		mcf.add_edge(2,4+i,1,R[1]-A[i]*B[i]*B[i]%R[1]);
		mcf.add_edge(3,4+i,1,R[2]-A[i]*B[i]*B[i]*B[i]%R[2]);
		mcf.add_edge(4+i,1000,1,A[i]*B[i]);
		mcf.add_edge(4+i,1000,1,A[i]*(B[i]*B[i]-B[i]));
		mcf.add_edge(4+i,1000,1,A[i]*(B[i]*B[i]*B[i]-B[i]*B[i]));
	}
	
	ll ret=mcf.mincost(0,1000,3*M)-(R[0]+R[1]+R[2])*M;
	cout<<-ret<<endl;
