...
void solve() {
	...
	FOR(i,N) if(A[i]) {
		vector<ll> V;
		FOR(j,A[i]+1) V.push_back(comb(A[i]-1,j)*factr[A[i]-j]%mo*fact[A[i]]%mo);
		Q.push(V);
	}
	...
	vec<ll> R=Q.front(),F;
	R.resize(N);
	FOR(i,N) R[i]=R[i]*fact[N-i]%mo;
	
	FOR(i,N) R[i]=R[i]*fact[i]%mo;
	FOR(i,N) F.push_back((i%2)?(mo-factr[i]):factr[i]);
	reverse(ALL(F));
	
	auto FR=MultPoly(F,R,1);
	FR.resize(3*N);
	queue<pair<vec<ll>,vec<ll>>> Q2;
	vector<ll> A(N);
	FOR(i,N) {
		A[N-1-i]=FR[N-1+i]*factr[i]%mo;
	}
	...
	while(Q2.size()>1) {
		auto a=Q2.front();
		Q2.pop();
		Q2.push(add(a,Q2.front()));
		Q2.pop();
	}
	vector<ll> pp=Q2.front().first;
	vector<ll> qq=Q2.front().second;
