...
ll dfs(int L,int R,int cur) {
	if(cur<L||cur>=R) return -1LL<<60;
	if(L>=R) return -1LL<<60;
	if(L+1==R) return 0;
	ll ret=0;
	auto p=st.getval(L,cur);
	auto q=st.getval(cur+1,R);
	ret=max(ret,dfs(L,cur,p.second)+cur-p.second);
	ret=max(ret,dfs(cur+1,R,q.second)+q.second-cur);
	return ret;
