...
void solve() {
	...
	cin>>H>>W;
	H--,W--;
	vector<ll> G(W+1),C(W+1),F(W+1);
	FOR(i,W+1) {
		G[i]=comb(2*i,i)*comb(2*i+H,H)%mo;
		C[i]=catalan(i);
	}
	vector<ll> GC=MultPoly(G,C,1);
	FOR(i,W+1) {
		F[i]=G[i];
		if(i) (F[i]+=2*(mo-GC[i-1]))%=mo;
	}
	
	ll ret=0;
	FOR(i,W+1) (ret+=F[i]*F[W-i])%=mo;
	ret=ret*comb(2*H,H)%mo;
	cout<<ret<<endl;
