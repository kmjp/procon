...
void solve() {
	...
	while(T--) {
		...
		FOR(i,N) {
			cin>>S[i];
			FOR(j,W) if(S[i][j]=='o'&&S[i][(j+1)%W]=='o') scc.add_edge(j*N+i,(j+1)%W*N+i);
		}
		FOR(i,M) {
			FOR(x,W) if(S[U[i]][x]=='o'&&S[V[i]][(x+1)%W]=='o') scc.add_edge(x*N+U[i],(x+1)%W*N+V[i]);
			FOR(x,W) if(S[V[i]][x]=='o'&&S[U[i]][(x+1)%W]=='o') scc.add_edge(x*N+V[i],(x+1)%W*N+U[i]);
		}
		scc.scc();
		
		
		
		if(scc.SC.size()!=N*W) cout<<"Yes"<<endl;
		else cout<<"No"<<endl;
	}
}
